        ...
        if var1.get()==True and var2.get()==False:
            ...
            if persona in os.listdir():
                ...
                if registros==0:
                    ...
                    cv2.imwrite(persona+str(len(registros))+".png",cv2.resize(frame,(224,224)))
...
xo,yo=500,50
#--------CANVAS
canv=Canvas(api,width=450,height=450)
...
capturar=Button(api,text="CAPTURAR",command=capturar)
...
while True:
    ...
    if OK==False:
        ...
        # convertir el array frame en imagen PIL
        image = pilimg.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        # convertir a  PhotoImage
